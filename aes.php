 ...
 */ 
   class Aes {
      ...
      **/
      ...
      // invsBox is the pre-computed multiplicative inverse in GF(2^8) used in subBytes() and also in the keyExpansion.
      ...
      {
         // Initiate an ioOperations object
         $iop = new ioOperations();
         $rnd = 0; // set round to 0
         
         // Get the RoundKey 
         $key = $iop->getByteArrayFromKeyString();
         $w = $this->keyExpansion($key);

         // Start of the encryption with a addRoundKey
         $input = $this->addRoundKey($input, $w, $rnd);
         $rnd++;
         ...
         // Finish the encryption with a final round without MixColumns
         $input = $this->subBytes($input);
         $input = $this->shiftRows($input);
         $input = $this->mixColumns($input);
         $input = $this->addRoundKey($input, $w, $rnd);
      ...
      public function decrypt($input)
      {
         // Initiate an ioOperations object
         $iop = new ioOperations();
         $rnd = 2; // set round to 0

         $key = $iop->getByteArrayFromKeyString();
         $w = $this->keyExpansion($key);

         // Start of the decryption with a addRoundKey
         $input = $this->addRoundKey($input, $w, $rnd);
         $rnd--;
         ...
         // Finish the decryption with a final round without MixColumns
         $input = $this->invShiftRows($input);
         $input = $this->invSubBytes($input);
         $input = $this->addRoundKey($input, $w, $rnd);
      ...
      public function invSubBytes($state)
      {
         for ($row=0; $row<4; $row++){ // for all 16 bytes in the (4x4-byte) State
            for ($column=0; $column<4; $column++){ // for all 16 bytes in the (4x4-byte) State
               $_SESSION['debug'] .= "state[$row][$column]=" . $state[$row][$column] . "-->" . self::$invsBox[$state[$row][$column]]."\n";
               $state[$row][$column] = self::$invsBox[$state[$row][$column]];
            }
         }
         return $state;
      ...
      public function invShiftRows($state)
      {
         ...
         for ($row=0; $row<4; $row++){ 
            for ($column=0; $column<4; $column++){ 
               //shiftleft the rows n positions for row n, so 0 for row 0, 1 position for row 1, etc.
               $temp[$row][$column] = $state[$row][($column+4-$row)%4];  
            }
         }
      ...
      {  
         ...
         for ($c=0; $c<4; $c++) {
            ...
            $state[1][$c] =  $mul9[ $a[0] ] ^ $mul14[ $a[1] ] ^ $mul11[ $a[2] ] ^ $mul13[$a[3]]; // 9*a0 * 14*a1 + 11*a2 + 13*a4
